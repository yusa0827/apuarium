...
リモートの最新状態と自分のブランチが自動マージ可能かどうか不安な場合は、以下のチェックも有効です。

```
python tools/check_merge_with_branch.py  # 省略時は origin/master とのマージを検証
```

内部で一時的なワークツリーを作成して `git merge --no-commit --no-ff` を試行し、コンフリクトがあれば終了コード1で知らせます。リモート名やブランチ名を引数で指定できるため、レビュー前のセルフチェックや CI に組み込んでおくと、マージ不能なプルリクを未然に防げます。
