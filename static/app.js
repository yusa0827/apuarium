...
const waterSurfaceMaterial = new THREE.ShaderMaterial({
  ...
  uniforms: {
    uTime: { value: 0 },
    uColorDeep: { value: new THREE.Color(0x114261) },
    uColorHighlight: { value: new THREE.Color(0x5ddcff) },
  },
  vertexShader: `
    uniform float uTime;
    varying vec2 vUv;
    ...
    void main() {
      vec3 pos = position;
      float waveX = sin((pos.x + uTime * 0.9) * 2.4);
      float waveY = cos((pos.y + uTime * 1.2) * 2.1);
      vWave = (waveX + waveY) * 0.5;
      pos.z += vWave * 0.08;
      vUv = uv;
  ...
  fragmentShader: `
    ...
    varying float vWave;
    void main() {
      float fresnel = pow(1.0 - abs(vWave) * 6.0, 2.0);
      vec3 color = mix(uColorDeep, uColorHighlight, 0.45 + vWave * 0.25 + vUv.y * 0.2);
      float alpha = 0.32 + fresnel * 0.25;
      gl_FragColor = vec4(color, clamp(alpha, 0.25, 0.85));
    }
  `,
});

const waterSurface = new THREE.Mesh(
  new THREE.PlaneGeometry(worldBounds.x, worldBounds.z, 80, 80),
  waterSurfaceMaterial,
);
waterSurface.rotation.x = -Math.PI / 2;
waterSurface.position.y = halfBounds.y - 0.01;
...
class GoldfishActor {
  ...
  applyLiveState(state) {
    ...
    const nz = this.depth;
    this.targetPosition.set(nx, ny, nz);
    this.targetScale = 0.72 + state.scale * 0.38;
    if (Number.isFinite(state.dir)) {
  ...
  update(dt, time) {
    ...
    const swimSpeed = 2.2 + this.velocity.length() * 0.3;
    this.swimPhase += dt * swimSpeed;
    const tailSwing = 0.35 + this.velocity.length() * 0.12;
    if (this.tail) {
...
const cameraBaseTarget = cameraTarget.clone();
let cameraPhase = Math.random() * Math.PI * 2;
...
function animate() {
  ...
  cameraPhase += dt * 0.25;
  const sway = Math.sin(cameraPhase);
  camera.position.x = cameraBasePosition.x + sway * 0.4;
  camera.position.y = cameraBasePosition.y + Math.sin(cameraPhase * 0.6) * 0.12;
  camera.position.z = cameraBasePosition.z + Math.cos(cameraPhase * 0.5) * 0.35;
  cameraTarget.y = cameraBaseTarget.y + Math.sin(cameraPhase * 0.4) * 0.05;
  camera.lookAt(cameraTarget);

  waterSurfaceMaterial.uniforms.uTime.value += dt;
